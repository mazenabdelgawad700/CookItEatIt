...
namespace RecipeApp.Core.Features.ApplicationUserFeature.Command.Handler
{
  ...
  IRequestHandler<ToggleFollowUserCommand, ReturnBase<bool>>
    {
...
";
        ...
        public async Task<ReturnBase<bool>> Handle(ToggleFollowUserCommand request, CancellationToken cancellationToken)
        {
            try
            {
                var toggleFollowingResult = await _applicationUserService.ToggleUserFollowAsync(request.FollowerId, request.FollowingId);
                if (!toggleFollowingResult.Succeeded)
                    return ReturnBaseHandler.Failed<bool>(toggleFollowingResult.Message);
                return ReturnBaseHandler.Success(true, toggleFollowingResult.Message);
            }
            catch (Exception ex)
            {
                return ReturnBaseHandler.Failed<bool>(ex.Message);
